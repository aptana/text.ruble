...
command 'AES Encrypt Document / Selection With Password...' do |cmd|
  ...
  cmd.invoke do |context|
    pw = RadRails::UI.request_secure_string(:title => 'Encrypt Text With AES 128', :prompt => 'What password should be used?', :button1 => 'Continue', :button2 => 'Cancel').to_s
    context.exit_discard if pw.nil? or pw.strip.length == 0
    pw2 = RadRails::UI.request_secure_string(:title => 'Encrypt Text With AES 128', :prompt => 'Enter password again to verify.', :button1 => 'Encrypt', :button2 => 'Cancel').to_s
